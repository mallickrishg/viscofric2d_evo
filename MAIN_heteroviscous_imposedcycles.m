...
etamax = 1e20;%Maxwell viscosity in Pa-s; for power>1, this is A^{-1}
etacore = 1e18;
corewidth = 10e3;
...
if burger == 0
    if power==1
        ...
        %etavalues = etacore + 2*(etamax-etacore)/pi.*atan2(abs(x2c),corewidth);
        etavalues = etamax.*ones(shz.N,1);
        etavalues(abs(x2c)<corewidth) = etacore;
        shz.eta = etavalues*1e-6;
    elseif power>1
...
axis tight equal
cb=colorbar;cb.Label.String = '\eta (Pa-s)';cb.Location = 'northoutside';
set(gca,'ColorScale','log','YDir','reverse','Fontsize',20)
subplot(212)
semilogy(x2c./1e3,etavalues,'x')
axis tight
ylabel('\eta (Pa-s)');
set(gca,'FontSize',20)
pause(0.01)
...
figure(6),clf
plotindex = abs(x2c)<corewidth;
plot(sqrt(e12d(end,plotindex).^2 + e13d(end,plotindex).^2)'.*triarea(plotindex)./mean(triarea(plotindex)),x3c(plotindex)./1e3,'s'), hold on
plot(sqrt(shz.e12pl(plotindex).^2 + shz.e13pl(plotindex).^2).*triarea(plotindex)./mean(triarea(plotindex)),x3c(plotindex)./1e3,'r.')
axis tight
ylabel('x_3 (km)')
